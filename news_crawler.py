...
def crawl_articles(collectedData):

    # Convert string to json object
    articles = json.loads(collectedData)
    # Create "articles" dictonary
    data = {'articles': []}
    ...
    for x in articles['articles']:
        ...
        if (newsName[1] == 'washingtonpost'):
            ...
            articleContainer = page_soup.findAll('div', {'class': 'entry__text js-entry-text bn-entry-text yr-entry-text'})
        ...
        article_details = {'source': x['source'], 'title': x['title'], 'article': json_article['article'], 'url': x['url']}
